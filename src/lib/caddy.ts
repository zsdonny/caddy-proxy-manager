...
import { parseAuthentikConfig, DEFAULT_AUTHENTIK_HEADERS, DEFAULT_AUTHENTIK_TRUSTED_PROXIES, type ProxyHostAuthentikMeta } from "./caddy-authentik";
export type { AuthentikRouteConfig } from "./caddy-authentik";
  ...
  for (const row of rows) {
    ...
    if (authentik) {
      ...
      if (authentik.protectedPaths && authentik.protectedPaths.length > 0) {
        ...
        // Excluded paths: protect the entire domain except the specified paths
        for (const domainGroup of domainGroups) {
          if (outpostRoute) {
            const outpostMatches = (outpostRoute.match as Array<Record<string, unknown>> | undefined) ?? [];
            hostRoutes.push({
          ...
          // Authenticated route: everything except excluded paths
          const authenticatedHandlers: Record<string, unknown>[] = [...handlers, forwardAuthHandler, reverseProxyHandler];
          hostRoutes.push({
            match: [
              {
                host: domainGroup,
                not: [{ path: authentik.excludedPaths }]
              }
            ],
            handle: authenticatedHandlers,
            terminal: true
          });

          // Unprotected fallback: catch-all for excluded paths
          const unprotectedHandlers: Record<string, unknown>[] = [...handlers, reverseProxyHandler];
          hostRoutes.push({
            match: [
              {
